...
def find_min_delta(vector_delta, vector_j, vector_jb):
    ...
    for i in vector_j_not_b:
        delta_vals.append(vector_delta[i-1])
...
def main_stage_simplex_method(m, n, matrix_a, vector_x, vector_c, vector_jb, logger=print):
    ...
    vector_j = {i for i in range(1, n + 1)}  # Множество индексов
    ...
    while True:
        ...
        try:
            ...
            if iteration == 1:
                ...
                matrix_ab_inverse = matrix_inversion(len(vector_jb), k + 1, matrix_ab_inverse, matrix_a[:, j0 - 1])
        ...
        vector_z = matrix_ab_inverse.dot(matrix_a[:, j0 - 1])
        log(f'\tz(T) = {vector_z}')
